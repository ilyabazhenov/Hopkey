...
/// Грубая, но честная проверка «комбинация скорее всего будет конфликтовать».
///
/// Carbon `RegisterEventHotKey` регистрирует и заведомо занятые сочетания (для `⌘C`
/// и т.п. возвращает `noErr`), а конфликт проявляется лишь при диспетчеризации события —
/// поэтому по статусу регистрации реальный конфликт почти не выявить. Зато можно
/// предупредить об опасных по конструкции комбинациях: глобальный хоткей без Control и
/// без Option почти наверняка перехватывает стандартные сочетания приложений
/// (`⌘C`/`⌘V`/`⌘W`, `⇧⌘Z` …). Рекомендуемый минимум для глобального хоткея — наличие
/// `⌃` или `⌥`.
public func hotKeyLikelyConflicts(modifiers: UInt32) -> Bool {
    guard modifiers != 0 else { return false }   // без модификаторов рекордер комбинацию и так не примет
    let safe = UInt32(controlKey) | UInt32(optionKey)
    return modifiers & safe == 0
}
